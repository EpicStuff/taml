	...
	config:
		...
		nested:
			...
			d:
				...
				g: [3, 4]
			extra_nested: 'unused'
		# comment before the repeated section
		servers:
			web:
				port: 80
				weights: [1, 2]
...
class Main(unittest.TestCase):
	...
	def test_baseline_parses(self) -> None:
		...
		# the trailing marker converts everything past the static prefix
		assert out['items'] == [1, 'r', 3, 4]
		# the value form, at the deepest nesting level
		assert out['config']['nested']['d']['g'] == [3, 4]
		# a repeated dictionary keeps its data keys, and repeat is not one of them
		assert list(out['config']['servers']) == ['web', 'backup']
		assert out['config']['servers']['web'] == {'port': 80, 'weights': [1, 2]}
		assert out['config']['servers']['backup'] == {'port': 8443, 'weights': []}
	...
	@parameterized.expand([
		...
		(
			'repeat_value_form_wants_list',
			DATA_OK.replace('g: [3, 4]', 'g: 5'),
			'Expected list or None, got int: 5 (line 18, col 7)',
		),
		(
			# the repeated entries move under an unused key so only the marker's own value is wrong
			'repeat_dict_wants_dict',
			DATA_OK.replace('servers:', 'servers: 5\n\tunused:'),
			'Expected dict or None, got int: 5 (line 21, col 11)',
		),
	])
	...
	@parameterized.expand([
		...
		# a repeated list nested inside a repeated dictionary
		('repeat_nested', ConversionValueError, 'weights: [1, 2]', "weights: [1, 'nope']", "Cannot convert 'nope' to int (line 24, col 17)"),
		('repeat_value_form', ConversionValueError, 'g: [3, 4]', "g: [3, 'nope']", "Cannot convert 'nope' to int (line 18, col 11)"),
	])
	...
	@parameterized.expand([
		...
		('depth1_items', "\t- 1\n\t- 'r'\n\t- 3\n\t- 4", '\t- 1', 'items[1] is required (line 29, col 1)'),
		# the path names the data key that repeat matched, not the marker
		('repeat_dict_key', '\t\t\tport: 8443\n', '', 'config.servers.backup.port is required (line 25, col 3)'),
		# the index continues past the static prefix instead of restarting at the repeat
		('repeat_list_index', '\t- 4', '\t- null', 'items[3] is required (line 33, col 4)'),
	])
