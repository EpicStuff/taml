...
from taml import ConversionTypeError, ConversionValueError, SchemaDefinitionError, always, taml
from utils import assert_equals, assert_raises, assert_raises2, raise_runtime_error, recorded_values, recording_converter, reset_recorded_values, return_none
...
class Main(unittest.TestCase):
	...
	def test_runs_for_every_present_falsy_value(self, _name, data, expected) -> None:
		reset_recorded_values()
		assert taml.loads(data, {'a': always(recording_converter)}) == {'a': expected}
		assert recorded_values == [(expected, None)]
	...
	def test_nested_null_runs_but_missing_key_does_not(self) -> None:
		schema = {'outer': {'a': always(recording_converter)}}
		reset_recorded_values()
	...
	def test_list_null_runs_but_missing_index_does_not(self) -> None:
		schema = {'items': [always(recording_converter)]}
		reset_recorded_values()
