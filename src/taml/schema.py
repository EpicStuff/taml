...
class Schema:
	@classmethod
	def from_taml(cls, path: str, line: int, col: int, func: Callable | None = None, _arg_cols: List[int] = ()) -> Self:
...
class required(Schema):
	...
	@classmethod
	def from_taml(cls, path: str, _line: int, _col: int, func: Callable | None = None, _arg_cols: List[int] = ()) -> Self:
		'Skip func check.'
		return cls(func, path)
...
class strict(Schema):  # noqa: PLW1641
	...
	def __init__(self, func: type, *funcs: type, _source: None | str = None) -> None:
		# "duplicate" func to ensure at least 1 func
		super().__init__((func, *funcs), _source)
	def __call__(self, data: Any, line: int, col: int, _path: str = '') -> Any:
		if data is not None and type(data) not in self.func:
...
class SchemaError(YAMLError):
	...
	# 		return f'partial({cls.name(value.func)})'
...
def _format_schema(schema: Dict | Any, line: int | None = None, col: int | None = None, path: str = '') -> Any | None:
	...
	if isinstance(schema, str):
		return _resolve(schema, line or 0, col or 0, path.lstrip('.'))  # pyright: ignore[reportArgumentType]
...
def _format_data(data: Any, schema: Any, lc: tuple[int, int] = (0, 0), plc: tuple[int, int] | None = None, path: str = '') -> Any:  # lc = line/col, plc = parent line/col
	...
	if isinstance(schema, Mapping):
		...
		for s_key, s_value in schema.items():  # schema key, schema value
			...
			if data is None or s_key not in data:
				_format_data(None, s_value, plc, path=f'{path}.{s_key}')
			else:
	...
	# if the schema value is list
		...
		for num, s_value in enumerate(schema):
			...
			if data is None or num >= len(data):
				_format_data(None, s_value, plc, path=f'{path}[{num}]')
			else:
	...
	else: assert data is None, 'tmp, look into this (schema should always be a callable at this point i think)'
	return data
